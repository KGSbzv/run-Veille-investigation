...
const CaseDetailPage: React.FC = () => {
    ...
    const handleAnalyzeFile = async (file: CaseFile) => {
        ...
        try {
            ...
            const resultJson: FileAnalysis = JSON.parse(cleanedResultText);
            ...
            console.error("Failed to parse Gemini response as JSON", e, "Response was:", resultText);
    ...
    if (!caseData || !caseId) return <div className="text-center text-dark-text-secondary">Dossier non trouvé.</div>;
    ...
    return (
        <div>
            ...
            <div className="mt-6">
                {activeTab === 'chat' && <CaseChat caseId={caseId} caseTitle={caseData.title} />}
                {activeTab === 'files' && (
                                    ...
                                    <div>
                                        ...
                                        {file.analysis && (
                                            <div className="mt-2 p-2 border-l-2 border-brand-accent bg-gray-900 text-xs">
                                                ...
                                                <p><strong>Risques:</strong> {file.analysis.risks.join(', ')}</p>
                                            </div>
                                        )}
                                    </div>
                                    <button
                                        onClick={() => handleAnalyzeFile(file)}
                                        disabled={analyzingFileId === file.id || !!file.analysis || !file.type.startsWith('image/')}
